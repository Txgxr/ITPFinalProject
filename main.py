...
class EditIconButton(Button):
    editIcon = StringProperty('C:\github\intro to programming\ITPFinalProject\Resources\Images\pencil_icon.png')

class TrashIconButton(Button):
    trashIcon = StringProperty('C:\github\intro to programming\ITPFinalProject\Resources\Images\\trash_icon.png')
...
class SplitsScreen(Screen, FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        for split in self.splitEntries:
            self.splitCount += 1
            self.add_widget(Button(text=f'{os.path.splitext(split)[0]}', pos_hint={'center_x':.5,'center_y':.75-(.1*(self.splitCount-1)*1.5)}, size_hint=(None, None), size=(450, 80), font_size=25, background_color=(0.294,0,1,0.509)))
            self.add_widget(EditIconButton(pos_hint={'center_x':.15, 'center_y':.75-(.1*(self.splitCount-1)*1.5)}, size_hint=(None, None), size=(80,80), background_color=(0.294,0,1,0.509)))
            self.add_widget(TrashIconButton(pos_hint={'center_x':.85, 'center_y':.75-(.1*(self.splitCount-1)*1.5)}, size_hint=(None, None), size=(80,80), background_color=(0.294,0,1,0.509)))
    ...
    def createSplit(self, splitName):
        if not os.path.exists(f'C:\github\intro to programming\ITPFinalProject\Resources\Splits\{splitName}.txt') and self.splitCount < 5:
            ...
            self.add_widget(Button(text=f'{splitName}', pos_hint={'center_x':.5,'center_y':.75-(.1*self.splitCount*1.5)}, size_hint=(None, None), size=(450, 80), font_size=25, background_color=(0.294,0,1,0.509)))
            editIconButton = EditIconButton(pos_hint={'center_x':.15, 'center_y':.75-(.1*self.splitCount*1.5)}, size_hint=(None, None), size=(80,80), background_color=(0.294,0,1,0.509))
            self.add_widget(editIconButton)
            trashIconButton = TrashIconButton(pos_hint={'center_x':.85, 'center_y':.75-(.1*self.splitCount*1.5)}, size_hint=(None, None), size=(80,80), background_color=(0.294,0,1,0.509))
